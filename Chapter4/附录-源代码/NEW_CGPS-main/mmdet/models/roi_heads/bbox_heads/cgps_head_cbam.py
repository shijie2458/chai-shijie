...
from mmdet.models.roi_heads.bbox_heads.cbam import CBAM
...
class CGPSHead(nn.Module):
    ...
    def forward(self, x1, x):
        ...
        # 创建CBAM模块实例
        ca_sa = CBAM(in_planes=x.size(1))  # 这里in_planes是x的通道数，可以根据需要调整
        # 对x和x1分别应用CBAM注意力模块
        x = x * ca_sa(x.view(x.size(0), x.size(1), 1, 1))  # 为x应用通道和空间注意力
        x1 = x1 * ca_sa(x1.view(x1.size(0), x1.size(1), 1, 1))  # 为x1应用通道和空间注意力
