...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
         # 将输入 x 转换为 [batch_size, channels, 1, 1] 形式，适配卷积层的要求
        x = x.view(x.size(0), x.size(1), 1, 1)  # 这里是调整形状
...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        ...
        # 使用通道注意力加权输入特征图
        out = x * self.ca(x.view(x.size(0), x.size(1), 1, 1))  
