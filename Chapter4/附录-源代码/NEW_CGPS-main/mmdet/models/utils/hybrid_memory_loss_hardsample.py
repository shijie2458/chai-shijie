...
class HybridMemoryMultiFocalPercent(nn.Module):
    ...
    def forward(self, results, indexes):
        inputs = results
        inputs = F.normalize(inputs, p=2, dim=1)
        # inputs.shape: torch.Size([样本数, 256])
        inputs = hm(inputs, indexes, self.features, self.momentum) #B*N, similarity
        # inputs.shape: torch.Size([样本数, 55272])
        inputs /= self.temp
        B = inputs.size(0)
        # 多焦点（multi-focal）策略和掩码操作
        def masked_softmax_multi_focal(vec, mask, dim=1, targets=None, epsilon=1e-6):
            ...
            masked_sums = masked_exps.sum(dim, keepdim=True) + epsilon
            return masked_exps / masked_sums
        # self.labels.shape: torch.Size([55272])
        targets = self.labels[indexes].clone()
        # targets.shape: torch.Size([27])
        ...
        mask = mask.expand_as(sim)
         ...
         # 改进点：动态负样本挖掘
        def hard_negative_mining(similarity, targets):
            neg_mask = torch.ones_like(similarity).bool()
            neg_mask[torch.arange(similarity.size(0)), targets] = False
            # 选择 top-k 难负样本
            _, hard_neg_indices = similarity.masked_select(neg_mask).view(similarity.size(0), -1).topk(k=5, dim=1)
            hard_neg_sim = similarity.gather(1, hard_neg_indices)
            return hard_neg_sim

        masked_sim = masked_softmax_multi_focal(sim.t().contiguous(), mask.t().contiguous(), targets=targets) #sim: u*B, mask:u*B, masked_sim: B*u

        # 使用动态难负样本
        hard_neg_sim = hard_negative_mining(sim, targets)
        logits = torch.cat([masked_sim, hard_neg_sim], dim=1)
        labels_extended = torch.cat([targets, targets + sim.size(1)])
         # 对比损失（InfoNCE 损失）(改进点：引入InfoNCE)
        contrastive_loss = F.cross_entropy(logits, labels_extended)
