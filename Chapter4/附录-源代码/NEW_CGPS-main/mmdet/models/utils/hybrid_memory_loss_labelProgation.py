...
class HybridMemoryMultiFocalPercent(nn.Module):
    def __init__(self, num_features, num_memory, num_classes=6000, temp=0.05, momentum=0.2, top_percent=0.1):
    ...
    def forward(self, results, indexes):
         ...
         # 使用标签传播来更新目标标签
        propagated_labels = self.label_propagation(results, self.labels, mask)
        print(f"[INFO]results:{results.shape}" )
        inputs = results
        inputs = F.normalize(inputs, p=2, dim=1)
        # inputs.shape: torch.Size([样本数, 256]),self.features 是存储在内存中的特征
        inputs = hm(inputs, indexes, self.features, self.momentum) #B*N, similarity
        ...
        targets = propagated_labels[indexes].clone()
        # indexes：要提取的样本的索引对应的标签
        # targets = self.labels[indexes].clone()
        # targets.shape: torch.Size([27])
        # self.labels：样本的标签，clone()为PyTorch 张量
        labels = self.labels.clone() #shape: N, unique label num: u
...
class LabelPropagation(nn.Module):
    def __init__(self, num_classes, num_neighbors=10, alpha=0.1, max_iter=100):
        ...
        self.num_classes = num_classes  # 标签的数量
        self.num_neighbors = num_neighbors # 每个样本在构建相似性图时考虑的邻居数
        self.alpha = alpha  # 标签传播的步长
        self.max_iter = max_iter  # 表示标签传播的最大轮次
    ...
    def forward(self, features, labels, mask):
        ...
        # 只传播未标记样本的标签
        propagated_labels = propagated_labels * mask.float()
