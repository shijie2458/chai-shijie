...
def run_training(cfg_file=None, eval_mode=False, resume=False, ckpt=None, opts=None):
    cfg = get_default_cfg()
    if cfg_file:
    ...
    if eval_mode:
        assert ckpt, "Checkpoint file must be specified for evaluation."
        resume_from_ckpt(ckpt, model)
    ...
    start_epoch = 0
    if resume:
        assert ckpt, "Checkpoint file must be specified to resume training."
        start_epoch = resume_from_ckpt(ckpt, model, optimizer, lr_scheduler) + 1
