...
from utils.utils import MetricLogger, SmoothedValue, mkdir, reduce_dict, warmup_lr_scheduler, save_on_master
...
def train_one_epoch(cfg, model, optimizer, data_loader, device, epoch, tfboard, softmax_criterion_s2,
                    softmax_criterion_s3, output_dir):
    model.train()
    ...
    for i, (images, targets) in enumerate(
        ...
        with autocast():
            ...
            # print(losses_reduced)#tensor(4.7675,device='cuda:0',grad_fn=<addBackward0>)
            loss_value = losses_reduced.item()  # item将张量转换为Python标量时，要求张量只包含一个元素torch.split
