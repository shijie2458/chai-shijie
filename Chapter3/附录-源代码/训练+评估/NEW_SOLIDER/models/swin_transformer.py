...
class PatchEmbed(BaseModule):
    ...
    def forward(self, x):
        ...
        x = x.flatten(2).transpose(1, 2)        # 压扁一维，改变顺序
...
class WindowMSA(BaseModule):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.embed_dims = embed_dims    # 维度是96分3个头，维度变为96*2，为了保持维度不变，头数变为3*2
        self.window_size = window_size  # Wh, Ww
        self.num_heads = num_heads      # 头数在每个stage数量增多（3，6，12，24）。
        head_embed_dims = embed_dims // num_heads
        self.scale = qk_scale or head_embed_dims**-0.5      # attention 中的根号dk
        self.init_cfg = init_cfg

        # define a parameter table of relative position bias 。B的形状要和前面保持一致，生成相对位置表格矩阵，数量是3*3,其中宽是num_heads
...
class SwinTransformer(BaseModule):
    ...
    def __init__(self,
                 pretrain_img_size=224,                         # 输入图像的大小img_size
                 in_channels=3,                                 # 输入通道数
                 embed_dims=96,                                 # 输出通道数
                 patch_size=4,                                  # 图像被分割成的小块（patch）的大小为 4x4
                 window_size=7,                                 # 窗口化注意力的窗口大小为 7x7
                 mlp_ratio=4,
                 depths=(2, 2, 6, 2),                           # 在每个阶段（stage）中堆叠的基本块（block）数量，依次为 [2, 2, 6, 2]
                 num_heads=(3, 6, 12, 24),                      # 在每个阶段（stage）中每个注意力头的数量，依次为 [3, 6, 12, 24]
                 strides=(4, 2, 2, 2),
                 out_indices=(0, 1, 2, 3),
                 qkv_bias=True,
                 qk_scale=None,
        ...
        if isinstance(pretrain_img_size, int):
            pretrain_img_size = to_2tuple(pretrain_img_size)            # 元组形式
        ...
        # patch embeding展平4x4x3=48,卷积操作：4*4的卷积核，步长为4，2，2，2
        ...
        if self.use_abs_pos_embed:              # 宽高切割patch
