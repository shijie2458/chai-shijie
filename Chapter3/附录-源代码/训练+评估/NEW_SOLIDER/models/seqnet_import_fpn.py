...
class SeqNet(nn.Module):
    def __init__(self, cfg
        ...
        reid_head = deepcopy(box_head)                                      # 创建完全独立于 box_head 的新对象 reid_head
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False):
        ...
        # print("type",type(images.tensors))
        # print("shape",images.tensors.shape)

        feature = self.backbone(images.tensors)            # images[2,3,1216,1504]>>[B,C,H,W]每张图片长宽不一样
        return_nodes = {'1': '0', '2': '1', '4': '2', '6': '3'}
        x = IntermediateLayerGetter(feature, return_layers=return_nodes)
        # for key, value in features.items():
        #     print(f"{key}: {value.shape}")                # feat_res4: torch.Size([2, 384, 54, 94])

        features = self.fpn(x)
        # backbone返回字典类型 ，打印OrderedDict对象的键值对
        proposals, proposal_losses = self.rpn(images, features, targets)
        _, detector_losses = self.roi_heads(features, proposals, images.image_sizes, targets)  # SeqRoIHeads
...
class SeqRoIHeads(RoIHeads):
    ...
    def forward(self, features, proposals, image_shapes, targets=None, query_img_as_gallery=False):
        ...
        if self.training:
            ...
            boxes = [boxes_per_image.detach() for boxes_per_image in boxes]         # detach()新的张量,不参与梯度运算
...
def detection_losses(
    ...
    loss_box_cls = F.binary_cross_entropy_with_logits(box_cls_scores, box_labels.float())   # 二元交叉熵损失
    ...
    def forward(self, x):
        ...
        for name, module in self.items():
            ...
            if name in self.return_layers:
                ...
                out[out_name] = torch.permute(x, [0, 3, 1, 2])  # 调整特征的维度顺序
        return out